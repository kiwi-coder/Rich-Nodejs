...
exports.Rich = function () {
    ...
    function prompt(message, callback) {
        ...
        stdin.once('data', function (input) {
            try {
                callback.call(caller, input.toString().trim());
            } catch (error) {
                console.log(error.message);
                prompt.call(caller, message, callback);
            }
        });
    };

    function promptInitMoney() {
        prompt.call(this, '设置玩家初始资金，范围1000～50000（默认10000）', this.initMoney);
    };

    this.start = function () {
        promptInitMoney.call(this);
    };

    function promptSelectPlayers() {
        prompt.call(this, '请选择2~4位不重复玩家，输入编号即可。(1.钱夫人; 2.阿土伯; 3.孙小美; 4.金贝贝):', this.selectPlayers);
    };

    this.initMoney = function (input) {
        _initMoney = input;
        promptSelectPlayers.call(this);
    };
