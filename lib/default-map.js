...
exports.getMap = function (cli) {
    var map = new Map(29, 8);

    map.setSite(0, new Site(new Cntl.StartSiteController()));

    for (var index = 1; index < 14; index++) {
        map.setSite(index, new Site(new Cntl.HouseSiteController(new House(200), cli)));
    }
    ...
    for (var index = 29; index < 35; index++) {
        map.setSite(index, new Site(new Cntl.HouseSiteController(new House(500), cli)));
    }

    map.setSite(35, new Site(new Cntl.GiftSiteController()));

    for (var index = 36; index < 49; index++) {
        map.setSite(index, new Site(new Cntl.HouseSiteController(new House(300), cli)));
    }
